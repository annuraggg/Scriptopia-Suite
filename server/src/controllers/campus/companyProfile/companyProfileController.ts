...
const createCompany = async (c: Context) => {
  try {
    ...
    if (!name) {
      return sendError(c, 400, "Company name is required");
    }
    ...
    const processedGeneralInfo = {
      ...generalInfo,
      studentsHired: Number(generalInfo.studentsHired),
      averagePackage: Number(generalInfo.averagePackage),
      highestPackage: Number(generalInfo.highestPackage),
    };
