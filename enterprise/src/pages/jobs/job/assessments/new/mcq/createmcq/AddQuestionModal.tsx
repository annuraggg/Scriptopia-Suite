...
import { Question, QuestionType, Option, AddQuestionModalProps } from '../../../../../../../types/mcq.types';
...
const QuestionModal: React.FC<AddQuestionModalProps> = ({
    ...
    useEffect(() => {
        if (editingQuestion) {
            ...
            const correctOption = editingQuestion.options?.find(opt => opt.isCorrect);
            if (correctOption) {
    ...
    const handleOptionChange = (id: number, field: 'text' | 'matchText' | 'isCorrect', value: string | boolean) => {
                        ...
                        {options.map((option) => (
                            <div key={option.id} className="flex items-center gap-2">
                                ...
                                <Checkbox
                                    ...
                                    onValueChange={(checked) => handleOptionChange(option.id, 'isCorrect', checked)}
                                >
                                    Correct
                                </Checkbox>
                            </div>
                        ...
                        {options.map((option) => (
                            <div key={option.id} className="flex items-center gap-2">
                                ...
                                <Input
                                    ...
                                    onChange={(e) => handleOptionChange(option.id, 'matchText', e.target.value)}
                                />
                            </div>
                        ))}
                    </div>
                ...
                <ModalFooter>
                    <div className="flex justify-between w-full">
                        <Button
                            ...
                            isDisabled={!["single-select", "multi-select", "matching", "output"].includes(selectedType || "")}
                        >
                            Add Option
                        </Button>
