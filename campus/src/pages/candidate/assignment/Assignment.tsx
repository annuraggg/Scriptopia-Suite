...
const Assignment = () => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!['application/zip', 'application/x-zip-compressed'].includes(selectedFile.type)) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await axios.post(`${import.meta.env.VITE_API_URL}/candidates/assignment/submit`, formData, {
  ...
  return (
    <div className="p-4 max-w-4xl mx-auto">
      <Card className="mb-6">
        <CardHeader>
          <CardTitle className='text-white text-xl'>{assignment?.name || "Assignment"}</CardTitle>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Upload your assignment as a .zip file (max 10MB)</CardDescription>
        </CardHeader>
        <CardContent>
